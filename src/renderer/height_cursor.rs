...
impl HeightCursor {
    ...
    pub(crate) fn vpos_adjust(
        ...
        // reflow 가 vpos 를 항상 계산하므로 합성/실측 구분(구현속성 태그)에 따른
        // 전방 보정 클램프는 제거한다 — 이전 문단 seg 의 vpos 를 무조건 신뢰한다.
        ...
        // 합성 seg 전방 클램프(SYNTH_FORWARD_REANCHOR)는 제거됐다 — reflow 된 vpos 를
        // 무조건 신뢰하므로 방향·크기 기반 차단 대역이 없다.
        result
    }
